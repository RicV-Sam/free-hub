...
test("Opportunity health report accepts a clean active enabled state", () => {
  ...
  writeText(path.join(rootDir, "opportunity", "fixture-current-sample", "index.html"), '<html><head><meta name="robots" content="noindex, follow"></head><body></body></html>');
  ...
  writeText(path.join(rootDir, "sitemap.xml"), "<urlset></urlset>");
  const dataPath = path.join(rootDir, "data", "opportunities.json");
  const rows = JSON.parse(fs.readFileSync(dataPath, "utf8"));
  rows[0].publicationStatus = rows[0].verificationStatus = "withdrawn";
  writeJson(dataPath, rows);
  const options = { rootDir, asOfDate: "2026-07-28", rawFeatureValue: "true", allowedSourceHosts: ["samples.example.org"] };
  assert.equal(buildOpportunityHealthReport(options).ok, false, "changing statuses alone must not clear an unresolved review");
  rows[0].withdrawnAt = rows[0].updatedAt = "2026-07-28";
  rows[0].withdrawalReason = "Official source no longer confirms the sample; keep its historical page closed.";
  writeJson(dataPath, rows);
  const withdrawn = buildOpportunityHealthReport(options);
  assert.equal(withdrawn.ok, true);
  assert.equal(withdrawn.records[0].evidence.sourceUrl.status, "stale");
  assert.match(withdrawn.reviewedWarnings.join("\n"), /fresh proof is required/);
  writeText(path.join(rootDir, "opportunity", "fixture-current-sample", "index.html"), '<html><head></head><body></body></html>');
  assert.equal(buildOpportunityHealthReport(options).ok, false, "withdrawn pages must stay noindex");
  rows[0].withdrawnAt = "2026-07-29";
  writeJson(dataPath, rows);
  assert.equal(buildOpportunityHealthReport(options).ok, false, "future or invalid withdrawals cannot resolve errors");
});
