...
function getCurrentContentBaseline() {
  ...
  return { active, core, publicOpportunities, detailOpportunities, samples, testing, featured,
    // Reviewed 18 September base includes the two website and eight Instagram additions;
    // active counts add the corresponding exit routes.
    generatedFileCount: 368 + active.length + read('data/student-guide.json').offers.length + detailOpportunities.length + publicOpportunities.length,
