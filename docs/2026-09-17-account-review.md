...
## Follow-up: stale account scripts after deployment

The user's subsequent failed request still omitted the newly added field. The live unversioned Firebase script contained the fix but was served with `Cache-Control: max-age=14400`. Account entry scripts and all six Firebase-client imports now use `20260917-account-save-v2` query versions, including the guest-ad auth observer, so a fresh page loads a consistent updated module graph. No browser storage, sign-in session or consent is cleared. Build, 119 lifecycle tests and three browser smoke tests passed again.
