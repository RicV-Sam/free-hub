...
The Free Stuff parent contains 25 durable resources in total; that count is separate from the seven Samples resources and the flag-controlled Opportunity registry.
...
- Telkom customer referral credit: R550 account credit for an existing active fixed or mobile customer after a referred fibre order goes into service; label it as a customer-only referral reward, not cash, a prize draw or a publisher-affiliate programme, and keep it separate from the personalised Mo'Nice prepaid promotion.
